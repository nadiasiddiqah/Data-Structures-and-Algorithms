...
// SOLVED MYSELF PARTIALLY (just the for loop)
func solution(_ S : String) -> String {
    ...
    let secondLastChar = chars.count - 2
    if chars[secondLastChar] == "-" {
        // swap: *-x change to -*x
        chars[secondLastChar] = chars[secondLastChar - 1]
        chars[secondLastChar - 1] = "-"
    }
    
    // remove last lingering dash
    if chars.last == "-" {
        chars.removeLast()
    }
